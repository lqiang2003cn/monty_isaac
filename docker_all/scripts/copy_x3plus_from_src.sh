#!/usr/bin/env bash
# Copy x3plus robot description (URDF + meshes) from an external source package
# into docker_all so that both Isaac Sim and ROS2 use the same model.
#
# Usage:
...
#   Expected layout: SOURCE_DIR/urdf/ and SOURCE_DIR/meshes/.
#
# Single source of truth:
#   - Meshes  -> shared/x3plus_isaac/meshes/   (canonical; Dockerfile COPYs into ros2 build)
#   - xacro   -> components/ros2_comp/app/monty_demo/x3plus_robot/urdf/  (source of truth for URDF)
#   - Isaac URDF is generated from the xacro via scripts/generate_isaac_urdf.sh
#
# After copying, you may need to replace package://lqtech_ros2_x3plus/meshes with
# package://monty_demo/meshes in the xacro if the source uses lqtech_ros2_x3plus.
...
SHARED_MESHES="$DOCKER_ALL/shared/x3plus_isaac/meshes"
...
echo "  urdf/* -> $X3PLUS_ROBOT/urdf/"
echo "  meshes/* -> $SHARED_MESHES/  (single source)"

mkdir -p "$X3PLUS_ROBOT/urdf" "$SHARED_MESHES"
cp -r "$SRC/urdf/"* "$X3PLUS_ROBOT/urdf/"
cp -r "$SRC/meshes/"* "$SHARED_MESHES/"
...
echo ""
echo "To regenerate the Isaac Sim URDF from the xacro (requires xacro / ROS 2):"
echo "  ./scripts/generate_isaac_urdf.sh"
