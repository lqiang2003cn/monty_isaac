...
From **docker_all** (or the repo root) with your Isaac Sim virtualenv activated:

```bash
python docker_all/components/ros2_comp/app/monty_demo/demos/hello_isaac.py
...
(If you are already in `docker_all`, use paths relative to that, e.g. `components/ros2_comp/app/monty_demo/demos/...`.)
...
Loads the X3plus robot from `monty_demo/x3plus_robot/urdf/x3plus.urdf`, resolves ROS `package://` mesh paths, and runs the simulation.

**Meshes required.** The URDF references meshes via `package://lqtech_ros2_x3plus/meshes/...`. Do one of the following:

- **Copy meshes into the repo:** Copy (or symlink) the `meshes` folder from the **lqtech_ros2_x3plus** package into `monty_demo/x3plus_robot/meshes/`, keeping the same structure (`X3plus/visual`, `X3plus/collision`, `sensor/visual`, `sensor/collision`). See `monty_demo/x3plus_robot/meshes/README.md`.
