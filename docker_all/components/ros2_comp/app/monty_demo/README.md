...
**Docker (recommended):** Build and run happen inside the `ros2_comp` container. From `docker_all/`: `docker compose build` then `./scripts/run_compose.sh` (or `docker compose up`). See `docker_all/README.md`.
