#!/usr/bin/env python3
"""Vision model server — runs in the vision_comp container.

Accepts JSON-line commands over a TCP socket, processes them with
SAM2/VGGT/Grounding DINO, and writes JSON-line responses back.
Binary data (images, masks, depths) is exchanged via numpy files on
a shared tmpfs volume (/vision_shm).

Clients (monty_comp) connect via ``monty_ext.vision._bridge.VisionBridge``.

Environment variables:
    VISION_PORT       TCP listen port (default 5570)
    MODEL_CACHE_DIR   Checkpoint directory (default /models)
    HF_HOME           HuggingFace cache (default /models/huggingface)
    TORCH_HOME        PyTorch hub cache (default /models/torch)
    VISION_SHM        Shared memory path (default /vision_shm)
...
Protocol (one JSON object per line, newline-delimited):
  ...
  -> {"cmd": "segment", "image": "/vision_shm/vb_frame.npy"}
  <- {"ok": true, "mask": "/vision_shm/vb_mask.npy"}
  ...
  <- {"ok": true,
     "masks": "/vision_shm/vb_video_masks.npy",
     "gdino_results": "/vision_shm/vb_gdino_results.npy"}

  -> {"cmd": "vggt", "images": "/vision_shm/vb_batch.npy",
     "masks": "/vision_shm/vb_masks.npy"}
  <- {"ok": true,
      "extrinsics": "/vision_shm/vb_ext.npy",
      "intrinsics": "/vision_shm/vb_intr.npy",
      "depths": "/vision_shm/vb_depths.npy",
      "points3d": "/vision_shm/vb_pts.npy"}

  -> {"cmd": "detect_gdino", "image": "/vision_shm/vb_frame.npy",
     "text_prompt": "red box"}
...
def _suppress_stdout():
    """Redirect stdout to stderr during model loading to avoid log noise."""
    saved = sys.stdout
...
def _check_model_cache():
    """Log which model checkpoints are cached and which are missing."""
    cache = os.environ.get("MODEL_CACHE_DIR", "/models")
    expected = {
        "sam2/sam2.1_hiera_large.pt": 898_083_611,
        "groundingdino/groundingdino_swint_ogc.pth": 650_000_000,
    }
    _log("Model cache check:")
    for relpath, min_size in expected.items():
        fpath = os.path.join(cache, relpath)
        if os.path.isfile(fpath) and os.path.getsize(fpath) >= min_size:
            _log(f"  cached: {relpath} ({os.path.getsize(fpath) / 1e6:.0f} MB)")
        else:
            _log(f"  MISSING: {relpath} (will download on first use)")

    hf_home = os.environ.get("HF_HOME", os.path.join(cache, "huggingface"))
    vggt_dir = os.path.join(hf_home, "hub", "models--facebook--VGGT-1B")
    if os.path.isdir(vggt_dir):
        _log(f"  cached: VGGT-1B (HuggingFace)")
    else:
        _log(f"  MISSING: VGGT-1B (will download on first use)")
...
def _dispatch(msg: dict) -> dict | None:
    """Route a command to its handler.  Returns None for shutdown."""
    cmd = msg.get("cmd", "")
    ...
    if cmd == "load_sam2":
        ...
        _load_gdino()
        return {"ok": True, "available": _gdino_model is not None}
    elif cmd == "segment":
        return _handle_segment(msg)
    elif cmd == "detect_gdino":
        return _handle_detect_gdino(msg)
    elif cmd == "segment_video":
        return _handle_segment_video(msg)
    elif cmd == "vggt":
        return _handle_vggt(msg)
    elif cmd == "shutdown":
        return None
    else:
...
def _serve_connection(conn: socket.socket, addr) -> bool:
    """Serve one client connection.  Returns False if shutdown requested."""
    _log(f"Client connected: {addr}")
    rfile = conn.makefile("r", buffering=1)
...
def main() -> None:
    ...
    srv.listen(1)
    _log(f"Listening on tcp://0.0.0.0:{port}")
    ...
    while not shutdown:
        ...
        if not _serve_connection(conn, addr):
            shutdown = True

    _log("Vision server shut down")
